...
def cadastro_medico(request):
    ...
    if is_medico(request.user):
        messages.add_message(request, constants.WARNING, 'Voce ja cadastrou como medico.')
...
def finalizar_consulta(request, id_consulta):
    ...
    if request.user != consulta.data_aberta.user:
        messages.add_message(request, constants.Error, 'Essa consulta so podera ser finalizada pelo(a) {{consulta.data_aberta.user}}')
...
def add_documento(request, id_consulta):
    ...
    if consulta.data_aberta.user != request.user:
        messages.add_message(request, constants.ERROR, 'Somente o(a) {{consulta.data_aberta.user}} podera adicionar documentos!')
        return redirect(f'/medicos/consulta_area_medico/{id_consulta}')
